...
int main (int argc, char *argv[])
{
  ...
  if ((queueDiscType != "BLUE") && (queueDiscType != "PfifoFast") && (queueDiscType != "GentleBLUE"))
    {
      NS_ABORT_MSG ("Invalid queue disc type: Use --queueDiscType=BLUE or --queueDiscType=PfifoFast");
    }
      if (queueDiscType == "GentleBLUE")
    {
      // Turn on Gentle BLUE
      Config::SetDefault ("ns3::BlueQueueDisc::GentleBlue", BooleanValue (true));
    }
  ...
  else if(queueDiscType == "GentleBLUE")
    {
      stack.Install (d.GetLeft ());
      stack.Install (d.GetRight ());
      TrafficControlHelper tchBottleneck;
